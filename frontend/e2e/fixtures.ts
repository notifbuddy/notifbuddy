...
// The backend resolves sessions by forwarding the cookie to authd — in this
// stack, the fake served by fakeapis, which verifies an HMAC-signed identity
// token it also minted and wrote to the shared volume as session.json. We read
// that here and seed the cookie into the browser context.
//
...
// Signed-in test: seeds the forged session cookie before each test so the
